...
export class ErlangRandomPolicy extends DistributionRandomPolicy {
	...
	distribution(upper: number, nsamples: number, rng: PRNG) {
		...
		for (let i = 0; i < nsamples; ++i) {
			...
			nums.push(-Math.log(u) / this.lambda);
		}
		...
		//
		// NB. this analytic form also removes the need for the old nsamples == 1 special case: normalizing to the observed min and max used to
		// divide by zero when there was only one sample, which is exactly what the web UI asks for.
